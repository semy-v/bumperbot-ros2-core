...
class SimpleController(Node):
    def __init__(self):
        ...
        self.speed_conversion_matrix_ = np.array([[self.wheel_radius_/2, self.wheel_radius_/2],
        ...
        self.get_logger().info("The speed conversion matrix is %s" % self.speed_conversion_matrix_)
    ...
    def velocityCallback(self, msg: TwistStamped):
        ...
        wheel_speed = np.matmul(np.linalg.inv(self.speed_conversion_matrix_), robot_speed)
    ...
    def jointCallback(self, msg: JointState):
        ...
        if dt_seconds <= 0:
            #Skip the iteration if not enough time has passed
            return
        ...
        quate = quaternion_from_euler(0, 0, self.theta_)
        self.publishOdometry(quate, linear, angular)
        self.publishTransform(quate)

    
    def publishOdometry(self, quate, linear, angular):
        # update odometry msg orientation
        self.odom_msg_.pose.pose.orientation.x = quate[0]
        self.odom_msg_.pose.pose.orientation.y = quate[1]
        self.odom_msg_.pose.pose.orientation.z = quate[2]
        self.odom_msg_.pose.pose.orientation.w = quate[3]
        self.odom_msg_.header.stamp = self.get_clock().now().to_msg()
        
        # update odometry msg position
        self.odom_msg_.pose.pose.position.x = self.x_
        self.odom_msg_.pose.pose.position.y = self.y_

        # update odometry msg velocity
    ...
    def publishTransform(self, quate):
        self.transform_stamped_.transform.translation.x = self.x_
        self.transform_stamped_.transform.translation.y = self.y_
        self.transform_stamped_.transform.rotation.x = quate[0]
        self.transform_stamped_.transform.rotation.y = quate[1]
        self.transform_stamped_.transform.rotation.z = quate[2]
        self.transform_stamped_.transform.rotation.w = quate[3]
