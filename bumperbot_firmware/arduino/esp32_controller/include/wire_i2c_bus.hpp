...
class WireI2cBus {
 public:
    ...
    bool readBlock(uint8_t address, uint8_t reg, std::span<uint8_t> buffer) {
        ...
        // requestFrom returns the actual number of bytes successfully read into the internal Wire
        // buffer
        const size_t bytes_received =
            wire_.requestFrom(static_cast<uint16_t>(address), static_cast<uint8_t>(buffer.size()),
        ...
        std::generate(buffer.begin(), buffer.end(),
                      [this]() { return static_cast<uint8_t>(wire_.read()); });
