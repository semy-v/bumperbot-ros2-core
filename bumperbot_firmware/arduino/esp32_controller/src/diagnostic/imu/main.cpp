...
void setup() {
    ...
    constexpr uint32_t kSerialWaitMs{10000U};
    ...
    const auto calibration = utils::retry<kMaxRetries>(
        [&delay_function] { return imu_sensor.calibrate(delay_function); });
    if (!calibration) {
    ...
    Serial.printf("DATA_RDY interrupt enabled at %u Hz on A6. Waiting for samples...\n",
...
void loop() {
    ...
    if ((sample_count % kPrintEverySamples) == 0U) {
        ...
        if (missed_notification_count != 0U) {
            Serial.printf("[WARNING]: %lu DATA_RDY notification(s) coalesced since startup.\n",
